...
from option_critic.nnet import Model
...
class MLP3D():
    def __init__(self, num_options, model_network, temp=1):
        ...
        self.options_b = theano.shared(
            np.zeros((num_options, model_network[-1]["out_size"])))
...
class OptionCritic_Network():
    def __init__(self, model_network=None, gamma=0.99, learning_method="rmsprop", actor_lr=0.00025,
        ...
        self.state_model = Model(
            state_network, input_size=input_size, dnn_type=dnn_type)
        self.state_model_prime = Model(
            state_network, input_size=input_size, dnn_type=dnn_type)
        output_size = [None, model_network[-2]["out_size"]]
        self.Q_model = Model(
            Q_network, input_size=output_size, dnn_type=dnn_type)
        self.Q_model_prime = Model(
            Q_network, input_size=output_size, dnn_type=dnn_type)
        self.termination_model = Model(
            termination_network, input_size=output_size, dnn_type=dnn_type)
        ...
        termination_sample = T.gt(
            option_term_prob, self.theano_rng.uniform(size=o.shape))
        ...
        sampled_actions = T.argmax(self.theano_rng.multinomial(
            pvals=action_probs, n=1), axis=1).astype("int32")
        ...
        critic_updates = learning_algo.apply(
            critic_params, grads, grad_clip=grad_clip)

        # actor updates
        actor_params = self.termination_model.params + self.options_model.params
        learning_algo = self.termination_model.get_learning_method(
            "sgd", lr=actor_lr)
        ...
        actor_updates = learning_algo.apply(
            actor_params, grads, grad_clip=grad_clip)
        ...
        self.train_critic = theano.function(
            [], [critic_cost], updates=critic_updates, givens=critic_givens)
        self.train_actor = theano.function(
            [s], [], updates=actor_updates, givens=actor_givens)
        self.pred_score = theano.function(
            [], T.max(Q, axis=1), givens={x: self.x_shared})
        self.sample_termination = theano.function(
            [s], [termination_sample, T.argmax(Q, axis=1)], givens={o: self.o_shared})
        self.sample_options = theano.function([s], T.argmax(Q, axis=1))
        self.sample_actions = theano.function(
            [s], sampled_actions, givens={o: self.o_shared})
