...
from option_critic.action_remapper import ActionRemapper
from option_critic.exp_replay import DataSet
from option_critic.neural_net import OptionCritic_Network
...
def filecreation(model_params, folder_name=None):
    tempdir = os.getcwd()
    create_dir(tempdir)
    folder_name = folder_name if folder_name is not None else datetime.datetime.now(
...
class Trainer(object):
    def __init__(self, model_params, ale_env, folder_name):
        ...
        self.action_counter = [{j: 0 for j in self.legal_actions}
                               for i in range(self.params.num_options)]

    def create_results_file(self):
        self.training_results_file = os.path.join(
            self.mydir, 'training_results.csv')
        data_file = open(self.training_results_file, 'wb')
        data_file.write('num_frame,episode_reward,time\n')
        data_file.close()

        self.training_results_ep_life_file = os.path.join(
            self.mydir, 'training_results_ep_life.csv')
        data_file = open(self.training_results_ep_life_file, 'wb')
        data_file.write('num_frame,episode_reward_ep_life,time\n')
        data_file.close()

        self.testing_results_file = os.path.join(
            self.mydir, 'testing_results.csv')
    ...
    def update_term_probs(self, epoch, term_probs):
        if self.params.nn_file is None:
            ...
            term_probs = term_probs if type(
                term_probs) is list else [term_probs]
    ...
    def update_stats(self, num_frame, current_option, current_action, since_last_term):
        ...
        if since_last_term is None:
            ...
            fd.write('%d,%d,%d,%d\n' % (num_frame, current_option,
                                        current_action, since_last_term))
        fd.close()
    ...
    def get_observation(self):
        screen = self.ale.getScreenGrayscale().reshape(
            self.screen_dims[1], self.screen_dims[0])

        if self.params.resize_method == "crop":
            resized = scipy.misc.imresize(screen, size=(110, 84))[
                self.params.offset:self.params.offset+84, :]
        elif self.params.resize_method == "scale":
            resized = cv2.resize(
                screen, (84, 84), interpolation=cv2.INTER_LINEAR)
        else:
    ...
    def save_model(self, total_reward, epoch_idx, skip_best=False):
        if total_reward >= self.best_reward and not skip_best:
            ...
            pkl.dump(self.model.save_params(), open(os.path.join(
                self.mydir, 'best_model.pkl'), "w"), protocol=pkl.HIGHEST_PROTOCOL)
        pkl.dump(self.model.save_params(), open(os.path.join(
            self.mydir, 'last_model.pkl'), "w"), protocol=pkl.HIGHEST_PROTOCOL)
        pkl.dump(self.model.save_params(), open(os.path.join(
            self.mydir, 'model.epoch-{}.pkl'.format(epoch_idx)), "w"), protocol=pkl.HIGHEST_PROTOCOL)
        print "Saved model"
    ...
    def get_epsilon(self):
        ...
        replay_start_time = max(
            [self.frame_count-self.params.replay_start_size, 0])
    ...
    def run_testing(self, epoch):
        ...
        while(self.frame_count - original_frame_count < self.params.steps_per_test):
            reward, fps = self.run_training_episode(
                self.max_frames_per_game, testing=True)
            ...
            print "rem:", rem,
            print "ETA: %d:%02d" % (
                max(0, rem/60/fps*4), ((rem/fps*4) % 60) if rem > 0 else 0),
    ...
    def train(self):
        ...
        for i in range(self.params.epochs):
            ...
            while self.frame_count-start_frames < self.params.steps_per_epoch:
                ...
                print "ETA: %d:%02d" % (
                    max(0, frames_rem/60/fps*4), ((frames_rem/fps*4) % 60) if frames_rem > 0 else 0),
                print "term ratio %.2f" % (100*self.term_ratio),
                print 'time: %.3f' % cur_time
                counter += 1
                ...
                self.update_training_results(
                    self.frame_count, total_reward, cur_time)
    ...
    def run_training_episode(self, max_steps, testing=False):
        ...
        while not game_over:
            ...
            if termination:
                ...
                since_last_term = 1
                current_option = np.random.randint(
                    self.params.num_options) if np.random.rand() < epsilon else new_option
            ...
            if self.print_option_stats:
                self.update_stats(self.frame_count, current_option,
                                  current_action, stat_since_last_term)
            ...
            reward, raw_reward, new_frame = self.act(
                current_action, testing=testing)

            game_over = self.ale.game_over() or (
                self.frame_count-start_frame_count) > max_steps
            new_num_lives = self.ale.lives()
            life_death = (
                new_num_lives < num_lives and not testing and self.params.death_ends_episode)
            num_lives = new_num_lives

            data_set.add_sample(x[-1], current_option,
                                reward, game_over or life_death)
            ...
            if not testing:
                if life_death or game_over:
                    ...
                    self.update_training_results_ep_life(
                        self.frame_count, episode_reward, cur_time)
        ...
        fps = round((self.frame_count - start_frame_count) /
                    (time.time()-start_time), 2)
        fps = self.ale.getEpisodeFrameNumber()/(time.time()-start_time)
        return total_reward, fps

    def print_table(self, conf_arr, d1):
        pkl.dump(np.array(conf_arr), open(
            "/".join(self.params.nn_file.split("/")[:-1])+"/confu_data.pkl", "wb"))
        ...
        plt.yticks(range(height), [self.legal_actions[iii]
                                   for iii in range(height)])
        plt.savefig("/".join(self.params.nn_file.split("/")[:-1])+"/"+self.params.nn_file.split(
            "/")[-2].replace(".", "_")+'_confu.png', bbox_inches='tight', format='png')
        raise NotImplemented

    def learn_actor(self, s, next_x, o, a, r, term):
        td_errors = self.model.train_conv_net(
            s, next_x, o, r, term, actions=a, model="actor")
        return td_errors

    def learn_critic(self):
        x, o, r, next_x, term = self.exp_replay.random_batch(
            self.params.batch_size)
        td_errors = self.model.train_conv_net(
            x, next_x, o, r, term, model="critic")
        return td_errors
...
class Q_Learning(DQN_Trainer):
    def __init__(self, **kwargs):
        ...
        self.exp_replay = DataSet(
            84, 84, self.rng, max_steps=self.params.replay_memory_size, phi_length=4)
