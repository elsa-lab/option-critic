...
def plot(filename, pdf_loc="training.pdf", csv_loc="training_progress.csv"):
    ...
    smooth_scores = [np.mean(scores[max(0, i-10):i+1])
                     for i in range(len(scores))]
    fig, ax1 = plt.subplots()
    ax1.plot(epochs, scores, "r", label="testing score")
    ax1.plot(epochs, smooth_scores, "g", label="10 moving avg")
    spread = (max(scores+[dqn_score, double_dqn_score]) -
              min(scores+[dqn_score, double_dqn_score]))*1.1
    if spread > 0:
        ax1.set_ylim([max(scores+[dqn_score, double_dqn_score]) -
                      spread, min(scores+[dqn_score, double_dqn_score])+spread])
