...
from option_critic.learning import get_learning_method
...
class AddBias():
    def __init__(self, init_val=None):
        self.b = init_val if init_val is not None else theano.shared(
            np.array([0.1], dtype="float32")[0])
...
class Model():
    ...
    def create_layer(self, inputs, model, dnn_type=True):
        ...
        if model["model_type"] == "conv":
            ...
            layer = conv_type(inputs,
                              ...
                              stride=stride,
                              nonlinearity=self.get_activation(
                                  model["activation"]),
                              W=lasagne.init.GlorotUniform(
                              ) if "W" not in model else inits[model["W"]],
            ...
            layer = lasagne.layers.DenseLayer(inputs,
                                              ...
                                              nonlinearity=self.get_activation(
                                                  model["activation"]),
                                              W=lasagne.init.GlorotUniform() if "W" not in model else (inits[model["W"]] if isinstance(
                                                  model["W"], basestring) else lasagne.init.Constant(model["W"])),
    ...
    def __init__(self, model, input_size=None, rng=1234, dnn_type=True):
        ...
        new_layer = tuple(input_size) if isinstance(
            input_size, list) else input_size
        print model
        self.model = model
    ...
    def apply(self, x, deterministic=False):
        ...
        for i, m in enumerate(self.model):
            ...
            last_layer_inputs = self.layers[i].get_output_for(
                last_layer_inputs, deterministic=deterministic)
